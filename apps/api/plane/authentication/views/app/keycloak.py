...
class KeycloakCallbackEndpoint(View):
    def get(self, request):
        ...
        app_host = request.session.get("host")
        ...
        if state != request.session.get("state", ""):
            ...
            url = urljoin(app_host, "?" + urlencode(params))
            return HttpResponseRedirect(url)
        ...
        try:
            ...
            return HttpResponseRedirect(urljoin(app_host, path))
        except AuthenticationException as e:
            params = e.get_error_dict()
            if next_path:
                params["next_path"] = str(validate_next_path(next_path))
            return HttpResponseRedirect(urljoin(app_host, "?" + urlencode(params)))
