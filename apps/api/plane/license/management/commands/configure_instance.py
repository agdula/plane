        ...
        if not InstanceConfiguration.objects.filter(key__in=keys).exists():
            for key in keys:
                ...
                if key == "IS_KEYCLOAK_ENABLED":
                    KEYCLOAK_ISSUER, KEYCLOAK_CLIENT_ID, KEYCLOAK_CLIENT_SECRET = get_configuration_value(
                        [
                            {
                                "key": "KEYCLOAK_ISSUER",
                    ...
                    if bool(KEYCLOAK_ISSUER) and bool(KEYCLOAK_CLIENT_ID) and bool(KEYCLOAK_CLIENT_SECRET):
                        value = "1"
                    else:
                        value = "0"
                    InstanceConfiguration.objects.create(
